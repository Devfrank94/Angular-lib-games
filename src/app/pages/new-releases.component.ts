  ...
  template: `
    @if (newReleasesLoading()) {
      ...
      <div class="glass p-2 rounded-xl shadow-xl">
        <div class="bg-base-300 rounded-xl shadow-xl px-2 sm:p-4 py-5 md:py-9">
          <!-- Navigazione Mese -->
          <nav
            class="flex justify-between items-center max-w-4xl mx-auto mb-6 p-4 bg-base-100 rounded-box shadow"
          >
            <button
          ...
          <!-- Legenda -->
          <div
            class="flex justify-center gap-6 mb-6 text-sm text-base-content/60"
          >
            <div class="flex items-center gap-2">
              <div
                class="w-4 h-4 rounded bg-primary/20 border border-primary"
              ></div>
              <span>Data di rilascio</span>
            </div>
            <div class="flex items-center gap-2">
              <div
                class="w-4 h-4 rounded bg-secondary/20 border border-secondary"
              ></div>
              <span>Oggi</span>
            </div>
          </div>
          ...
          <div class="max-w-6xl mx-auto">
            <div class="grid grid-cols-7 gap-1 md:gap-2">
              ...
              @for (day of weekdays; track day) {
                <div
                  class="text-center py-2 md:py-3 text-xs md:text-sm font-semibold text-primary bg-base-100 rounded"
                >
                  {{ day }}
                </div>
              }
              ...
              @for (day of calendarDays(); track day.date.toISOString()) {
                ...
                >
                  ...
                  <span
                    ...
                    [class.text-primary]="
                      day.releases.length > 0 && day.isCurrentMonth
                    "
                  >
                    {{ day.dayNumber }}
                  </span>
                  ...
                  @if (day.releases.length > 0) {
                    <div class="my-1 flex flex-wrap gap-1">
                      @for (
                        release of day.releases.slice(0, 3);
                        track release.id
                      ) {
                        <div
                      ...
                      @if (day.releases.length > 3) {
                        <div
                          class="w-6 h-6 md:w-12 md:h-12 avatar rounded-full bg-base-300 flex items-center justify-center text-xs font-bold"
                        >
                ...
                <div class="flex-center-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p class="font-semibold">
                      {{ day.releases.length }} Rilasc{{
                        day.releases.length > 1 ? 'i' : 'io'
                      }}
                    </p>
                  </div>
                </div>
                    ...
                    >
                      <figure class="w-1/3 sm:w-1/2 shrink-0">
                        <img
                          ...
                          [alt]="game.name"
                          class="h-full object-contain sm:object-cover"
                          (error)="handleImageError($event)"
                        />
                      </figure>
                      <div class="card-body p-3 md:p-4">
                        <h3
                          class="card-title text-sm md:text-base line-clamp-1"
                        >
                          {{ game.name }}
                        </h3>
                        ...
                        @if (game.metacritic) {
                          <div class="flex items-center gap-2 mt-auto">
                            <span class="text-xs text-base-content/80 font-bold"
                              >Metacritic</span
                            >
                            <span
                        ...
                        <div>
                          <a
                            ...
                            class="btn text-lg sm:text-md bg-accent text-white mt-3 shadow-md"
                          >
                            Dettagli Gioco
                          </a>
                        </div>
...
export default class NewReleasesComponent {
  ...
  constructor() {
    effect(() => {
      ...
      untracked(() => {
        this.apiService.getNewReleases(date.getFullYear(), date.getMonth());
      });
    });
  }
