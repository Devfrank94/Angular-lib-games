...
@Component({
  ...
  imports: [
    ...
    GameTrailerComponent,
    AchievementsComponent,
  ],
  template: `
    <div class="card bg-base-300 shadow-xl rounded-lg relative">
      <div>
        <figure>
          <button
            class="btn btn-base-300 sm:btn-rounded text-md btn-sm sm:btn-md mb-3 absolute top-2 left-2 sm:top-5 sm:left-5 z-1"
            (click)="goBack()"
          >
            <svg
            ...
            <div class="ml-2 hidden sm:block">Indietro</div>
          </button>
          <img
      ...
      <div class="card-body p-5 lg:p-8">
        <h1 class="card-title md:text-3xl">
          {{ game().name }}
          <span
            *ngIf="isNewGame()"
            data-theme="light"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          <div>
            ...
            <ul class="space-y-3 text-md">
              ...
              <li>
                <strong>Data ultimo aggiornamento:</strong>
                {{ game()?.updated || 'N/A' | date: 'dd-MM-yyyy' }}
              </li>
              <li><app-rating-star [rating]="game().rating" /></li>
              <li>
                <strong>Metacritic:</strong
                ><span class="ms-2 badge badge-warning badge-xs lg:badge-md">{{
                  game().metacritic || 'N/A'
                }}</span>
              </li>
              <li>
                <strong>Tempo di gioco:</strong> {{ game().playtime }} ore
              </li>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              @for (genre of game().genres; track genre.id) {
                <span class="badge text-white sm:badge-lg badge-accent">{{
                  genre.name
                }}</span>
              }
            </div>
            ...
            <div
              class="flex flex-wrap gap-3 mb-3 w-fit bg-accent py-2 px-3 sm:py-2 sm:px-4 rounded-full"
            >
              @for (
                parent of game().parent_platforms;
                track parent.platform.id
              ) {
                <a
                  class="tooltip tooltip-hover tooltip-bottom"
                  [attr.data-tip]="parent.platform.name"
                >
                  <img
            ...
            <div>
              @if (game()?.website) {
                ...
                <a
                  ...
                  ><p class="truncate">{{ game().website }}</p></a
                >
              }
            </div>
          </div>
        ...
        <div
          class="tabs tabs-lg tabs-box my-4 flex justify-center max-w-8xl mx-auto"
        >
          <label class="tab">
            <input
              type="radio"
              name="game_detail_tabs"
              (change)="showAchievements.set(true)"
            />
            <svg
            ...
            Obiettivi ({{ apiService.achievementsResponse()?.count ?? 0 }})
          </label>
          ...
          <label class="tab">
            <input
              type="radio"
              name="game_detail_tabs"
              (change)="showScreenshots.set(true)"
            />
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
            ...
            Screenshots ({{ apiService.screenshotsResponse()?.count ?? 0 }})
          </label>
          <div class="tab-content bg-base-100 border-base-300 p-6">
            @if (showScreenshots()) {
        ...
        @if (game()?.description) {
          <div class="mt-6">
            ...
            <p class="mb-2 text-xs truncate">
              (Le descrizioni potrebbero essere in lingua straniera)
            </p>
            <div
...
export class CardDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.apiService.getGameAchievements(gameId);
    this.apiService.getGameScreenshots(gameId);
  }
  ...
  gameId = toSignal(this.route.params.pipe(map((params) => +params['id'])));
